...
public class FileUploadTool {
	//视频桶
	private static final String bucketName="usa-video";
	...
	public Video_List createFile(MultipartFile multipartFile, HttpServletRequest request,String path,String playUrl) {
		System.err.println("视频上传");
		Video_List entity = new Video_List();
	    boolean bflag = false;
...
//	      String newFileName = this.getName(fileName);
	      ...
	      try {
			  ...
			  File uploadFile = new File(fileNamedirs);
			  String uploadKey = newFileName;
			  resulturl=upload.uploadToS3(uploadFile,uploadKey,bucketName);
			  Log4jUtil.info("上传到s3桶完毕，返回结果："+resulturl);
			  if (filedirs.exists()) {
				  filedirs.delete();
			  }
...
//	            System.out.println("设置转换为flv格式后文件的保存路径"+codcFilePath);
//	            //截图保存路径 
//	            String mediaPicPath = logoRealPathDir + File.separator + newFileName + ".jpg";
//	            System.out.println("截图保存路径"+mediaPicPath);
//	            // 获取配置的转换工具（ffmpeg.exe）的存放路径
////	            String ffmpegPath = request.getSession().getServletContext().getRealPath("/ffmpeg/ffmpeg.exe");
//	            transfMediaTool.processFLV(ffmpegPath, aviPath,  codcFilePath,mediaPicPath);
...
//	          }
//	        }
	        entity.setVideo_size(size);
	        String video_url=playUrl+File.separator + newFileName + fileEnd;
//	        entity.setVideo_url(video_url);
//	        entity.setSave_url(fileNamedirs);
	        entity.setVideo_url(resulturl);
	        entity.setSave_url(resulturl);
	        entity.setVideo_old_name(name);
	        entity.setVideo_new_name(newFileName);
	        return entity;
	      } else {
	        return null;
	      }
